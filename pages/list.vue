    ...
    <div class="grid row-auto col-start-1 w-full gap-2">
        ...
        >
          ...
          <div class="actions mt-4">
            ...
            <button @click="handleCallDates(profile.id)">
              {{ !showDates ? 'Visualizar horários disponivéis' : 'Fechar horários' }}
            </button>
          </div>
          ...
          <div class="bg-slate-600 p-2 flex gap-x-2 rounded-xl" v-if="dateAvailabe.length > 0">
            <div class="day flex flex-col gap-y-2 p-2" v-for="(date, index) in dateAvailabe" :key="index">
              ...
              <ul>
                <li v-for="(hour, hourIndex) in date.available_time" :key="hourIndex" class="flex flex-col items-center">
                  <button :for="hour"
